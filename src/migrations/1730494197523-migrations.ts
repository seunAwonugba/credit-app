...
export class Migrations1730494197523 implements MigrationInterface {
    name = 'Migrations1730494197523'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`transaction\` (\`id\` int NOT NULL AUTO_INCREMENT, \`transactionType\` enum ('debit', 'credit') NOT NULL, \`status\` enum ('success', 'failed', 'pending') NOT NULL, \`action\` enum ('fund', 'transfer', 'withdraw') NOT NULL, \`userId\` varchar(255) NOT NULL, \`amount\` decimal(10,2) NOT NULL DEFAULT '0.00', \`balanceBefore\` decimal(10,2) NOT NULL DEFAULT '0.00', \`balanceAfter\` decimal(10,2) NOT NULL DEFAULT '0.00', \`referenceId\` varchar(36) NOT NULL, \`metadata\` json NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`accountId\` int NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
